...
Creates a new captain account. The endpoint validates the payload (including vehicle details), hashes the password, stores the captain in the database, and returns a JWT token plus the created captain object (password excluded).
...
- `vehicle.type` — one of: `car`, `bike`, `scooter`, `van`, `auto-rickshaw`
...
| Status | Meaning | Example body |
|---|---|---|
| **201 Created** ✅ | Captain created successfully | `{ "token": "<jwt>", "captain": { "_id": "...", "fullname": {"firstname":"Raj","lastname":"Kumar"}, "email":"raj@example.com", "vehicle": {"color":"Red","plate":"KA01AB1234","capacity":4,"type":"car"}, "status":"inactive" } }` |
| **400 Bad Request** ⚠️ | Validation failed | `{ "errors": [ { "msg": "Invalid email format", "param": "email", "location": "body" } ] }` |
| **500 Internal Server Error** ⚠️ | Unexpected server or DB error | `{ "error": "error message" }` |
...
```bash
curl -X POST http://localhost:4000/api/captains/register \
  -H "Content-Type: application/json" \
  -d '{"fullname":{"firstname":"Raj","lastname":"Kumar"},"email":"raj@example.com","password":"Password123","vehicle":{"color":"Red","plate":"KA01AB1234","capacity":4,"type":"car"}}'
```
